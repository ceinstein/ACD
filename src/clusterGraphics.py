...
def displaySet():
	...
	def getDict():
		...
		try:
			...
			DICT = CFH.loadDictionary(dic.get())
	...
	def saveDict():
		...
		try:
			...
			CFH.saveDictionary(DICT, filename)
			dic.set(filename)
	...
	def getMatrix():
		...
		try:
			...
			if filename.endswith('.matrix'):
				MATRIX = CFH.loadMatrix(filename)
				matrix.set(filename)
	...
	def saveMat():
		...
		try:
			...
			CFH.saveMatrix(MATRIX, filename)
			matrix.set(filename)
	...
	def getLabels():
		...
		try:
			...
			if filename.endswith('.lbls'):
				LABELS = CFH.loadLabels(filename)
	...
	def saveLabs():
		...
		try:
			...
			CFH.saveLabels(LABELS, filename)
			labelHolder.set(filename)
