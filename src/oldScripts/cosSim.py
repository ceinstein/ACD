...
def generateMatrix(dictionary, logLength, log):
...
if __name__ == "__main__":
	...
	#neo = generateMatrix(dictionary, len(log), log)
	#neo = normalize(neo)
	#printMatrix(neo)
	#labels = KMeansCalc(neo)
	#simMatrix = generateSimilarties(neo)
	#topicModel(log, labels)


	#Compares each of the different clustering methods

	#kClusters = KMeansCalc(simMatrix, 3)
	#print 'KMEANS'
	#topicModel(log, kClusters)
	#print '\n\nGRAPH CLUSTER'
	#clusters, adjMatrix = graphCluster(log, simMatrix, neo)
	#CSG.networkGraphics(log, adjMatrix)
	#graphTopics(log, clusters)
	
	#print '\n\nNEW METHOD'
	#graphTopics(log, avgClusters)
	

	#print '\n\nNEW METHOD WITHOUT GRAPH CLUSTER'
	#time1 = time.time()
	#clusters, avgClusters = ACD(log, neo)
	#time2 = time.time()
	#print time2 - time1
	#print clusters
	#printMatrix(avgClusters)
	#topics = graphTopics(log, clusters)

	#averages, clusters = DACD.distributedACD(log, dictionary, neo, sys.argv[2])
	#topics = graphTopics(log, clusters)
	
	if(sys.argv[1] == 'w'):
		CSG.displaySet()
	else:
		log = CSFH.loadLog(sys.argv[2])
		dictionary = CSFH.loadDictionary(sys.argv[3])
		neo = CSFH.loadMatrix(sys.argv[4])
		simMatrix = generateSimilarties(neo)
		#simMatrix = CSFH.loadMatrix(sys.argv[5])
		clusters = generateNetwork(log, simMatrix)
		graphTopics(log, clusters)
